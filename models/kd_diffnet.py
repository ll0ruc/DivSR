...
class kd_diffnet():
    ...
    def constructTrainGraph(self):
        ...
        user_embedding_from_consumed_items = self.generateUserEmebddingFromConsumedItems(item_embedding)

        if self.conf.social == 1:
            gcn_user_embedding = user_embedding
            gcn_embeddings = [gcn_user_embedding]
            for _ in range(self.conf.gcn_layer):
                gcn_user_embedding = self.generateUserEmbeddingFromSocialNeighbors(gcn_user_embedding)
                gcn_embeddings.append(gcn_user_embedding)
            self.final_user_embedding = tf.reduce_sum(gcn_embeddings, axis=0) + user_embedding_from_consumed_items
        else:
            ## Without Social
            self.final_user_embedding = user_embedding + user_embedding_from_consumed_items
        ...
        self.reg_loss = self.conf.regu * (tf.nn.l2_loss(init_user_latent) + tf.nn.l2_loss(init_item_latent) + tf.nn.l2_loss(init_neg_latent))
