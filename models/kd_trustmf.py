...
class kd_trustmf():
    ...
    def initializeNodes(self):
        ...
        self.truster_input = tf.placeholder("int32", [None, 1])
        self.trustee_input = tf.placeholder("int32", [None, 1])
        self.trust_label = tf.placeholder("float32", [None, 1])
        ...
        self.w_embedding = tf.Variable(
            tf.random_normal([self.conf.num_users, self.conf.dimension], stddev=0.01), name='trustee_embedding')
        if self.conf.kd == 1:
    ...
    def constructTrainGraph(self):
        ...
        if self.conf.social == 1:
            user_embedding_from_social = self.generateUserEmbeddingFromSocialNeighbors(w_embedding)
            final_user_embedding += user_embedding_from_social
        ...
        if self.conf.social == 1:
            latest_truster_latent = tf.gather_nd(self.user_embedding, self.truster_input)
            latest_trustee_latent = tf.gather_nd(self.w_embedding, self.trustee_input)
            social_predict_vector = tf.multiply(latest_truster_latent, latest_trustee_latent)
            social_predict = tf.reduce_sum(social_predict_vector, 1, keepdims=True)
            social_loss = tf.nn.l2_loss(social_predict - self.trust_label)
            social_reg_loss = self.conf.regu * (tf.nn.l2_loss(latest_truster_latent)+tf.nn.l2_loss(latest_trustee_latent))
            self.opt_loss += social_reg_loss
            self.opt_loss += self.conf.regt * social_loss
    ...
    def defineMap(self):
        ...
        map_dict['train'] = {
            ...
            self.truster_input: 'TRUSTER_LIST',
            self.trustee_input: 'TRUSTEE_LIST',
